...
class PRSummarizer:
    ...
    async def run(self, repo: str, pr_number: int) -> PRSummary:
        ...
        summary = summary.strip().removeprefix("```json").strip("`").strip()
...
#         git=GithubProvider("Perfeed"), llm=OllamaClient("llama3.1:8b")
#     )
#     pr_summary = asyncio.run(summarizer.run("perfeed", 6))
#     print(pr_summary)
